...
# here we don't install h5py
RUN conda install -y numpy pyyaml scipy ipython mkl scikit-learn matplotlib pandas setuptools Cython graphviz libgcc mkl-include cmake cffi typing cython && \
...
RUN pip install tqdm regex pyyaml opencv-python opencv-contrib-python azureml-defaults future tensorboard && \
    pip install scikit-image 'pillow<7.0.0' shapely easydict jpeg4py numba ray==0.8.7 ray[tune] hyperopt yacs && \
